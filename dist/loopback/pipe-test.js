...
async function main() {
    ...
    await new Promise((r) => sink.listen(FAKE_RPC, "127.0.0.1", () => r()));
    ...
    const tel = await conn.request({ t: "hello", nodeId: id.nodeId, name: "tester", build: "test" }, (m) => m.t === "telemetry", 5000);
    if (tel.t !== "telemetry")
        throw new Error("no telemetry received");
    ...
    const joined = await conn.request({ t: "join", sessionId: "test", build: "test" }, (m) => m.t === "joined", 5000);
