...
program
    ...
    .addHelpText("before", `
  ${c.bold(c.cyan("weld"))} ${c.gray("v" + VERSION)}  ${c.gray(sym.dot)} ${c.gray("pool GPUs over llama.cpp rpc")}
`)
    .addHelpText("after", `
${c.gray("  quick start")}
    ...
    ${c.bold("weld status")} ${c.gray(sym.dot)} ${c.bold("weld doctor")}      ${c.gray("see the cluster / diagnose")}
...
models
    ...
    .action(run((q) => cmdModels("search", q)));
models
    .command("files <repo>")
    .description("list gguf files in a repo")
    .action(run((repo) => cmdModels("files", repo)));
models
    .command("download <repo> <file>")
    .description("download a gguf (resumable)")
    .action(run((repo, file) => cmdModels("download", repo, file)));
models
    .command("import <path>")
    .description("register a gguf you already have")
    .action(run((path) => cmdModels("import", path)));
...
peers
    ...
    .action(run((name) => cmdPeers("forget", name)));
program
    .command("run")
    ...
    .option("--exclude <name>", "skip a peer (repeatable)", (v, a) => [...a, v], [])
    .action(run(cmdRun));
function run(fn) {
    return async (...args) => {
        try {
