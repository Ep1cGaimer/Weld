...
class Reader {
    ...
    take(n) {
        ...
        const inWindow = this.pos >= this.bufStart && this.pos + n <= this.bufStart + this.buf.length;
        if (!inWindow) {
    ...
    int(bytes, signed) {
        ...
        if (bytes <= 6)
            return signed ? b.readIntLE(0, bytes) : b.readUIntLE(0, bytes);
    ...
    str() {
        const len = this.u64();
        if (len > 64 * 1024 * 1024)
            throw new Error("gguf: implausible string length");
...
export function parseGgufHeader(file) {
