...
import { banner, bullet, c, fmtBytes, fmtMB, info, kv, line, ok, Progress, rule, Spinner, step, table, } from "../ui.js";
export async function cmdModels(cmd, a, b) {
    ensureWeldHome();
    if (cmd === "list")
        return list();
    if (cmd === "search" && a)
...
async function search(q) {
    ...
    try {
        ...
        table(models.slice(0, 15).map((m) => [c.bold(m.id), c.gray(m.downloads.toLocaleString() + " dl")]), ["REPO", "DOWNLOADS"]);
        line();
...
async function files(repo) {
    ...
    try {
        ...
        table(list.map((f) => [c.bold(f.name), c.cyan(f.size ? fmtBytes(f.size) : "?")]), ["FILE", "SIZE"]);
        line();
