...
import { binariesReady, installedBuild, listDevices, llamaServerPath, llamaVersion, PINNED_BUILD, rpcServerPath, } from "../core/binaries.js";
...
export async function cmdDoctor() {
    ...
    if (build === PINNED_BUILD)
        ok(`build ${build} matches pin`);
    else {
    ...
    if (devices.length)
        ...
        warn("no accelerator devices reported (cpu only)");
    const tel = collectLocal();
    kv("ram free", `${fmtMB(tel.ramFreeMB)} / ${fmtMB(tel.ramTotalMB)}`);
    for (const g of tel.gpus)
        kv(g.name, `${fmtMB(g.vramFreeMB)} free / ${fmtMB(g.vramTotalMB)}`);
    line();
    rule("ports");
    for (const [label, port] of [
        ...
        if (free)
            ...
            warn(`${label} ${port} in use (fine if weld node / llama-server is running)`);
    }
    line();
    rule("peers");
    const peers = loadPeers();
    if (peers.length === 0)
        warn("none paired — run: weld pair");
    for (const peer of peers) {
        ...
        try {
            ...
            conn.close();
            s.succeed(`${c.bold(peer.name)} ${c.gray(peer.host)} — ${rtt}ms ${rtt < 2 ? c.green("(excellent)") : rtt < 10 ? c.green("(good)") : c.yellow("(high latency)")}`);
        }
        catch (e) {
            s.failed(`${c.bold(peer.name)} ${c.gray(peer.host)} — unreachable ${c.gray("(is `weld node` running there?)")}`);
            problems++;
        }
    }
    line();
    ...
    if (!existsSync(paths.catalog))
        warn("no models yet — weld models search <query>");
    else
        ok(`catalog at ${paths.catalog}`);
    line();
    if (problems === 0)
