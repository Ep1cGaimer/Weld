...
import { JsonLines } from "../core/proto.js";
import { banner, box, bullet, c, confirm, fail, info, kv, line, ok, rule, Spinner, step, warn, } from "../ui.js";
export function lanIps() {
    const ips = [];
    for (const list of Object.values(networkInterfaces())) {
        for (const i of list ?? []) {
            if (String(i.family) === "IPv4" && !i.internal)
                ips.push(i.address);
        }
    }
    return ips;
}
export async function cmdPair(code, opts) {
    ensureWeldHome();
...
async function dial(code, host, port) {
...
async function listen(controlPort) {
    const id = getIdentity();
    const cfg = loadConfig();
    ...
    bullet(c.bold(`weld pair ${code} --host ${ips[0] ?? "<your-ip>"}`) +
    ...
    await new Promise((resolve) => {
        const server = tls.createServer({ cert: id.certPem, key: id.keyPem }, (sock) => {
            ...
            handler = async (m) => {
                ...
                spinner.stop();
                const peer = peerFromCert(m.certPem, m.name, sock.remoteAddress || "127.0.0.1", m.controlPort, m.relayPort);
