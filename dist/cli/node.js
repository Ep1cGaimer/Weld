...
import { spawnProc, waitForPort } from "../proc/supervisor.js";
import { banner, box, bullet, c, fail, fmtMB, info, kv, line, ok, rule, step, warn, } from "../ui.js";
import { lanIps } from "./pair.js";
export async function cmdNode() {
    ensureWeldHome();
    ...
    const server = new ControlServer({
        ...
        onJoin: async (_sessionId, peerName) => {
            ...
            rpc = spawnProc(rpcServerPath(), ["--host", "127.0.0.1", "--port", String(cfg.rpcPort), "-c"], { env: { LLAMA_CACHE: paths.rpcCache } });
    ...
    box([
        ...
        `${c.gray("devices")}   ${tel.gpus.length ? tel.gpus.map((g) => `${g.name} ${fmtMB(g.vramFreeMB)} free`).join(", ") : "cpu only"}`,
    ...
    line();
    const telemetryTimer = setInterval(() => server.broadcast({ t: "telemetry", ...collectLocal(rpc !== null) }), 2000);
    const shutdown = () => {
