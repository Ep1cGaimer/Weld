...
export function loadCatalog(): CatalogEntry[] {
  return readJson<CatalogEntry[]>(paths.catalog, []).filter((e) => existsSync(e.path));
}
...
export function describe(path: string): CatalogEntry {
...
export function upsert(entry: CatalogEntry): void {
  saveCatalog([...loadCatalog().filter((e) => e.id !== entry.id), entry]);
}

export function findModel(idOrPath: string): CatalogEntry | null {
  const byId = loadCatalog().find((e) => e.id === idOrPath || e.file === idOrPath);
  if (byId) return byId;
  const abs = resolve(idOrPath);
  if (existsSync(abs) && abs.toLowerCase().endsWith(".gguf")) return describe(abs);
  return null;
}
