...
function deviceLabel(t: Telemetry): string {
  if (t.gpus.length === 0) return c.gray("cpu only");
...
export async function cmdStatus(): Promise<void> {
  ...
  const pool =
    poolOf(cluster.head) + cluster.live.reduce((acc, l) => acc + poolOf(l.telemetry), 0);
  rule("capacity");
  bullet(
    `${c.bold(c.cyan(fmtMB(pool)))} pooled across ${c.bold(String(1 + cluster.live.length))} node(s)`
  );
  ...
  const mismatch = [cluster.head, ...cluster.live.map((l) => l.telemetry)].filter(
    (t) => t.build !== PINNED_BUILD
  );
  if (mismatch.length) {
    line();
