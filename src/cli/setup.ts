...
export async function cmdSetup(): Promise<void> {
  ...
  if (devices.length === 0) bullet("none reported (cpu only)");
  for (const d of devices) bullet(d);
  ...
  kv("location", dir);
  if (!binariesReady()) fail("llama-server or rpc-server missing — rerun weld setup");

  if (process.platform === "win32") {
    line();
    info("if peers cannot reach you, allow weld through the firewall (admin shell):");
    bullet(
      `netsh advfirewall firewall add rule name="weld" dir=in action=allow protocol=TCP localport=7100,7101`
    );
  }
  line();
  step("next: weld pair");
...
async function fetchAsset(url: string, dest: string, label: string): Promise<void> {
